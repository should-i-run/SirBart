...
class FileWatcher extends EventEmitter {
  constructor(rootConfigs) {
    ...
    this._loading = Promise.all(rootConfigs.map(createWatcher)).then(watchers => {
  ...
  isWatchman() {
    return detectingWatcherClass.then(Watcher => Watcher === sane.WatchmanWatcher);
  }

  end() {
    return this._loading.then(watchers =>
      watchers.map(watcher => Promise.denodeify(watcher.close).call(watcher)),
    );
  }
