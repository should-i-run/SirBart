...
describe('processRequest', () => {
  ...
  pit('returns JS bundle source on request of *.bundle', () => {
    return makeRequest(requestHandler, 'mybundle.bundle?runModule=true').then(response =>
      expect(response).toEqual('this is the source'),
    );
  });

  pit('returns JS bundle source on request of *.bundle (compat)', () => {
    return makeRequest(requestHandler, 'mybundle.runModule.bundle').then(response =>
      expect(response).toEqual('this is the source'),
    );
  });

  pit('returns sourcemap on request of *.map', () => {
    return makeRequest(requestHandler, 'mybundle.map?runModule=true').then(response =>
      expect(response).toEqual('"this is the source map"'),
    );
  });

  pit('works with .ios.js extension', () => {
    return makeRequest(requestHandler, 'index.ios.includeRequire.bundle').then(response => {
      expect(response).toEqual('this is the source');
  ...
  pit('passes in the platform param', function() {
    return makeRequest(requestHandler, 'index.bundle?platform=ios').then(function(response) {
      expect(response).toEqual('this is the source');
      expect(Bundler.prototype.bundle).toBeCalledWith('index.js', true, 'index.map', true, 'ios');
    });
  });
  ...
  describe('buildbundle(options)', () => {
    it('Calls the bundler with the correct args', () => {
      ...
      expect(Bundler.prototype.bundle).toBeCalledWith('foo file', true, undefined, true, undefined);
    });
  });
