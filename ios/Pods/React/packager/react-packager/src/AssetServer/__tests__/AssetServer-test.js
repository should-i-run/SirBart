...
describe('AssetServer', () => {
  ...
  describe('assetServer.get', () => {
    pit('should work for the simple case', () => {
      ...
      return Promise.all([server.get('imgs/b.png'), server.get('imgs/b@1x.png')]).then(resp =>
        resp.forEach(data => expect(data).toBe('b image')),
      );
    });
    ...
    pit('should work for the simple case with platform ext', () => {
      ...
      return Promise.all([
        server.get('imgs/b.png', 'ios').then(data => expect(data).toBe('b ios image')),
        server.get('imgs/b.png', 'android').then(data => expect(data).toBe('b android image')),
        server.get('imgs/c.png', 'android').then(data => expect(data).toBe('c android image')),
    ...
    pit('should work for the simple case with jpg', () => {
      ...
      return Promise.all([server.get('imgs/b.jpg'), server.get('imgs/b.png')]).then(data =>
        expect(data).toEqual(['jpeg image', 'png image']),
      );
    });
    ...
    pit('should pick the bigger one', () => {
      ...
      return server.get('imgs/b@3x.png').then(data => expect(data).toBe('b4 image'));
    });
    ...
    pit('should pick the bigger one with platform ext', () => {
      ...
      return Promise.all([
        ...
        server.get('imgs/b@3x.png', 'ios').then(data => expect(data).toBe('b4 ios image')),
      ]);
    });
    ...
    pit('should support multiple project roots', () => {
      ...
      return server.get('newImages/imgs/b.png').then(data => expect(data).toBe('b1 image'));
    });
  });
