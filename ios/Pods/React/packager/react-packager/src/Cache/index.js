...
class Cache {
  constructor(options) {
    ...
    this._persistEventually = _.debounce(this._persistCache.bind(this), 2000);
  }
  ...
  _set(filepath, field, loaderPromise) {
    ...
    record.data[field] = loaderPromise
      .then(data => Promise.all([data, Promise.denodeify(fs.stat)(filepath)]))
      .then(([data, stat]) => {
  ...
  _persistCache() {
    ...
    var allPromises = _.values(data).map(record => {
      ...
      return Promise.all(fieldValues).then(ref => {
        ...
        fieldNames.forEach((field, index) => (ret.data[field] = ref[index]));
