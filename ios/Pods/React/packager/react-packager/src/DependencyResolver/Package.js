...
class Package {
  ...
  isHaste() {
    return this._cache.get(this.path, 'haste', () => this._read().then(json => !!json.name));
  }

  getName() {
    return this._cache.get(this.path, 'name', () => this._read().then(json => json.name));
  }
  ...
  redirectRequire(name) {
    return this._read().then(json => {
      ...
      const redirect = browser[relPath] || browser[relPath + '.js'] || browser[relPath + '.json'];
      if (redirect) {
  ...
  _read() {
    if (!this._reading) {
      this._reading = this._fastfs.readFile(this.path).then(jsonStr => JSON.parse(jsonStr));
    }
