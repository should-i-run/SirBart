...
class HasteMap {
  ...
  processFileChange(type, absPath) {
    return Promise.resolve().then(() => {
      ...
      if (this._helpers.extname(absPath) === 'js' || this._helpers.extname(absPath) === 'json') {
  ...
  _processHasteModule(file) {
    ...
    return module
      .isHaste()
      .then(
        isHaste => isHaste && module.getName().then(name => this._updateHasteMap(name, module)),
      );
  }
  ...
  _processHastePackage(file) {
    ...
    return p
      .isHaste()
      .then(isHaste => isHaste && p.getName().then(name => this._updateHasteMap(name, p)))
      .catch(e => {
