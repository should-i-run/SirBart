...
class ResolutionRequest {
  constructor({ platform, entryPath, hasteMap, deprecatedAssetMap, helpers, moduleCache, fastfs }) {
  ...
  resolveDependency(fromModule, toModuleName) {
    ...
    const asset_DEPRECATED = this._deprecatedAssetMap.resolve(fromModule, toModuleName);
    if (asset_DEPRECATED) {
    ...
    return Promise.resolve().then(() => {
      ...
      const collect = mod => {
        ...
        return mod
          .getDependencies()
          .then(depNames =>
            Promise.all(depNames.map(name => this.resolveDependency(mod, name))).then(
              dependencies => [depNames, dependencies],
            ),
          )
      ...
      .then(() => {
        ...
        return mod
          ...
          .then(bundles =>
            Promise.all(
              bundles.map(bundle =>
                Promise.all(bundle.map(dep => this.resolveDependency(mod, dep))),
              ),
            ).then(bs => bs.map(bundle => bundle.map(dep => dep.path))),
          );
      })
      .then(asyncDependencies =>
        asyncDependencies.forEach(dependency => response.pushAsyncDependency(dependency)),
      );
  }
  ...
  _resolveHasteDependency(fromModule, toModuleName) {
    ...
    return p.then(realModuleName => {
      ...
      if (dep && dep.type === 'Package') {
        const potentialModulePath = path.join(dep.root, path.relative(packageName, realModuleName));
        return this._tryResolve(
  ...
  _resolveNodeDependency(fromModule, toModuleName) {
    if (toModuleName[0] === '.' || toModuleName[1] === '/') {
      ...
      return this._redirectRequire(fromModule, potentialModulePath).then(realModuleName =>
        this._tryResolve(
      ...
      return this._redirectRequire(fromModule, toModuleName).then(realModuleName => {
        const searchQueue = [];
        for (
        ...
        searchQueue.forEach(potentialModulePath => {
          p = this._tryResolve(
            () => this._tryResolve(() => p, () => this._loadAsFile(potentialModulePath)),
            () => this._loadAsDir(potentialModulePath),
          );
        });
    ...
    return Promise.resolve().then(() => {
      ...
      if (this._fastfs.fileExists(packageJsonPath)) {
        return this._moduleCache
          ...
          .getMain()
          .then(main =>
            this._tryResolve(() => this._loadAsFile(main), () => this._loadAsDir(main)),
          );
      }
