...
class Fastfs extends EventEmitter {
  ...
  findFilesByExt(ext, { ignore }) {
    return this.getAllFiles()
      .filter(file => file.ext() === ext && (!ignore || !ignore(file.path)))
      .map(file => file.path);
  }
  ...
  findFilesByName(name, { ignore }) {
    return this.getAllFiles()
      .filter(file => path.basename(file.path) === name && (!ignore || !ignore(file.path)))
      .map(file => file.path);
  }
  ...
  closest(filePath, name) {
    for (let file = this._getFile(filePath).parent; file; file = file.parent) {
