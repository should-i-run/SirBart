...
describe('Bundler', function() {
  ...
  beforeEach(function() {
    ...
    require('../../JSTransformer').prototype.loadFileAndTransform.mockImpl(function(path) {
      return Promise.resolve({
  ...
  pit('create a bundle', function() {
    return bundler.bundle('/root/foo.js', true, 'source_map_url').then(function(p) {
      ...
      expect(p.addModule.mock.calls[2][0]).toEqual({
        code: 'lol module.exports = ' + JSON.stringify(imgModule_DEPRECATED) + '; lol',
        sourceCode: 'module.exports = ' + JSON.stringify(imgModule_DEPRECATED) + ';',
      ...
      expect(p.finalize.mock.calls[0]).toEqual([{ runMainModule: true }]);
  ...
  pit('gets the list of dependencies from the resolver', function() {
    return bundler
      .getDependencies('/root/foo.js', true)
      .then(() => expect(getDependencies).toBeCalledWith('/root/foo.js', { dev: true }));
  });
});
