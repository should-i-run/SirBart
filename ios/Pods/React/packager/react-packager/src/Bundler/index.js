...
class Bundler {
      ...
      .then(response => {
        ...
        return Promise.all(
          response.dependencies.map(module =>
            this._transformModule(bundle, response, module, platform).then(transformed => {
              if (bar) {
                bar.tick();
              }
              return transformed;
  ...
  _transformModule(bundle, response, module, platform = null) {
    ...
    if (module.isAsset_DEPRECATED()) {
      ...
      transform = this._transformer.loadFileAndTransform(path.resolve(module.path));
    }
  ...
  generateAssetModule_DEPRECATED(bundle, module) {
    return Promise.all([sizeOf(module.path), module.getName()]).then(([dimensions, id]) => {
      const img = {
  ...
  generateAssetModule(bundle, module) {
    ...
    return Promise.all([sizeOf(module.path), this._assetServer.getAssetData(relPath)]).then(
      function(res) {
...
function getPathRelativeToRoot(roots, absPath) {
  ...
  throw new Error('Expected root module to be relative to one of the project roots');
}
