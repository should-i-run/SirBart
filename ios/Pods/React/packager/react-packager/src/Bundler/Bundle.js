...
class Bundle {
  ...
  getMinifiedSourceAndMap() {
    ...
    try {
      ...
      if (e.line) {
        errorMessage += 'Transformed code line: "' + source.split('\n')[e.line - 1] + '"\n';
      }
      if (e.pos) {
        ...
        if (fromIndex > -1) {
          ...
          errorMessage +=
            'Module name (best guess): ' + source.substring(fromIndex, toIndex) + '\n';
        }
      }
  ...
  getSourceMap(options) {
    ...
    const map = {
      ...
      names: [],
      mappings: mappings,
      sourcesContent: options.excludeSource ? [] : _.pluck(this._modules, 'sourceCode'),
    };
    return map;
  }
    ...
    for (let i = 0; i < modules.length; i++) {
      ...
      for (let t = 0; t < code.length; t++) {
        if (t === 0 && i === 0) {
          ...
          mappings += base64VLQ.encode(0 - moduleLines[modules[i - 1].sourcePath]);
          mappings += 'A';
