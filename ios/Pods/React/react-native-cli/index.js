...
var CLI_MODULE_PATH = function() {
  return path.resolve(process.cwd(), 'node_modules', 'react-native', 'cli');
};
...
if (cli) {
  ...
  switch (args[0]) {
    ...
    default:
      console.error(
        'Command `%s` unrecognized. ' + 'Did you mean to run this inside a react-native project?',
        args[0],
      );
      process.exit(1);
      break;
...
function validatePackageName(name) {
  ...
  if (name === 'React') {
    console.error(
      '"%s" is not a valid name for a project. Please do not use the ' + 'reserved word "React".',
      name,
    );
    process.exit(1);
  }
