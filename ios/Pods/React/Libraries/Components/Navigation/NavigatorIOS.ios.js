...
var NavigatorIOS = React.createClass({
  ...
  _tryLockNavigator: function(cb: () => void) {
    this.refs[TRANSITIONER_REF].requestSchedulingNavigation(acquiredLock => acquiredLock && cb());
  },
  ...
  _routeToStackItem: function(route: Route, i: number) {
    ...
    return (
      <StaticContainer key={'nav' + i} shouldUpdate={shouldUpdateChild}>
        ...
        >
          <Component navigator={this.navigator} route={route} {...route.passProps} />
        </RCTNavigatorItem>
      </StaticContainer>
    );
  },
  ...
  renderNavigationStackItems: function() {
    var shouldRecurseToNavigator =
      this.state.makingNavigatorRequest || this.state.updatingAllIndicesAtOrBeyond !== null;
  ...
  render: function() {
    return <View style={this.props.style}>{this.renderNavigationStackItems()}</View>;
  },
});
